    ...
    for episode in range(num_episodes):
        ...
        for step in range(num_steps_per_episode):
            ...
            if agent.last_shape_info:
                ...
                prev_severity = (
                    episode_data[-1]["severity"] if episode_data else float("inf")
                )
                is_corrective = (
                    not shape_info["ok"]
...
if "ready" in st.session_state and st.session_state.ready:
    ...
    with col1:
        ...
        axes[0].scatter(
            steps[safe], beta_N[safe], c="green", s=20, alpha=0.7, label="Safe"
        )
        axes[0].scatter(
        ...
        axes[0].axhspan(
            constraints.BETA_N_MIN, constraints.BETA_N_MAX, alpha=0.15, color="green"
        )
        ...
        axes[1].scatter(
            steps[safe], q_min[safe], c="green", s=20, alpha=0.7, label="Safe"
        )
        axes[1].scatter(
            steps[violations & ~corrective],
        ...
        axes[1].axhspan(
            constraints.QMIN_MIN, max(q_min) * 1.1, alpha=0.15, color="green"
        )
        ...
        axes[2].scatter(
            steps[safe], q95[safe], c="green", s=20, alpha=0.7, label="Safe"
        )
        axes[2].scatter(
            steps[violations & ~corrective],
        ...
        axes[2].axhspan(
            constraints.Q95_MIN, constraints.Q95_MAX, alpha=0.15, color="green"
        )
    ...
    with col2:
        ...
        st.metric(
            "🔴 Violations", violation_count, f"{violation_count/total_steps*100:.1f}%"
        )
        st.metric(
            "🟠 Self-Fixing Actions",
            corrective_count,
            f"{corrective_count/total_steps*100:.1f}%",
        )
        ...
        ax.fill_between(
            steps, 0, severity, where=(severity > 0), alpha=0.3, color="red"
        )
        ax.set_xlabel("Step")
    ...
    # Mark points
    ax.scatter(
        beta_N[safe], q95[safe], c="green", s=30, alpha=0.7, label="Safe", zorder=3
    )
    ax.scatter(
        beta_N[violations & ~corrective],
        q95[violations & ~corrective],
    ...
    with col_anim1:
        animation_speed = st.slider(
            "Animation Speed", 0.1, 5.0, 1.0, 0.1, key="anim_speed"
        )
    with col_anim2:
        ...
        for frame in range(len(df)):
            ...
            if frame > 0:
                ...
                if np.any(frame_safe):
                    ax1.scatter(
                        frame_beta[frame_safe],
                        frame_q95[frame_safe],
                        c="green",
                        s=40,
                        alpha=0.7,
                        label="Safe",
                ...
                if np.any(frame_violations & ~frame_corrective):
                    ax1.scatter(
                        frame_beta[frame_violations & ~frame_corrective],
                ...
                if np.any(frame_corrective):
                    ax1.scatter(
                        frame_beta[frame_corrective],
                        frame_q95[frame_corrective],
                        c="orange",
                        s=100,
                        marker="*",
                        label="Self-Fixing!",
            ...
            # Add status to title
            status_emoji = (
                "🟠" if current_corrective else ("🟢" if current_ok else "🔴")
            )
            status_text_title = (
                "Self-Fixing!"
                if current_corrective
                else ("Safe" if current_ok else "Violation")
            )
            ax1.set_title(
                f"Live Trajectory - Step {frame+1}/{len(df)} | {status_emoji} {status_text_title}",
                fontsize=13,
                fontweight="bold",
            )
            ...
            for i in range(4):
                ax2.plot(
                    [x_safe[i], x_safe[i]],
                    [y_safe[i], y_safe[i]],
                    [constraints.Q95_MIN, constraints.Q95_MAX],
                    "g--",
                    alpha=0.4,
                    linewidth=1,
                )
            ...
            color = (
                "green" if current_ok else ("orange" if current_corrective else "red")
            )
    ...
    with st.expander("📋 Step-by-Step Details (Click to expand)"):
        ...
        display_df["status"] = display_df.apply(
            lambda row: (
                "🟢 Safe"
                if row["ok"]
                else ("🟠 Self-Fixing" if row["corrective"] else "🔴 Violation")
            ),
            axis=1,
        )
