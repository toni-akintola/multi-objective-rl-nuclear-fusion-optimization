...
def run(agent: Agent, num_episodes=10, track_shape=False, interactive=False):
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        while not (terminated or truncated):
            ...
            if interactive and getattr(agent, "last_shape_info", None):
                ...
                prev_severity = shape_history[-1]["severity"] if (track_shape and shape_history) else float('inf')
                is_corrective = (not shape_info["ok"] and prev_severity != float('inf')
        ...
        print(f"Episode {episode + 1}/{num_episodes}: Reward = {episode_reward:.2f}, Steps = {steps}{violation_info}")
    ...
    # Rewards
    ax1.plot(range(1, num_episodes + 1), episode_rewards, marker="o", linestyle="-", linewidth=2)
    ax1.axhline(y=np.mean(episode_rewards), color="r", linestyle="--", label=f"Mean: {np.mean(episode_rewards):.2f}")
    ...
    # Episode lengths
    ax2.plot(range(1, num_episodes + 1), episode_lengths, marker="s", linestyle="-", linewidth=2, color="green")
    ax2.axhline(y=np.mean(episode_lengths), color="r", linestyle="--", label=f"Mean: {np.mean(episode_lengths):.1f}")
    ...
    print(f"{'='*50}")
    print(f"Reward  - Mean: {np.mean(episode_rewards):>10.2f} ± {np.std(episode_rewards):.2f}")
    print(f"        - Min:  {np.min(episode_rewards):>10.2f}")
    print(f"        - Max:  {np.max(episode_rewards):>10.2f}")
    print(f"Steps   - Mean: {np.mean(episode_lengths):>10.1f} ± {np.std(episode_lengths):.1f}")
...
def visualize_shape_self_fixing(shape_history, agent_name="Agent"):
    ...
    ax1.set_xlabel('Step'); ax1.set_ylabel('β_N'); ax1.set_title('β_N: Self-Fixing Trajectory')
    ...
    ax2.set_xlabel('Step'); ax2.set_ylabel('q_min'); ax2.set_title('q_min: Self-Fixing Trajectory')
    ...
    ax3.set_xlabel('Step'); ax3.set_ylabel('q95'); ax3.set_title('q95: Self-Fixing Trajectory')
    ax3.legend(fontsize=7, loc='best'); ax3.grid(True, alpha=0.3)
    ...
    ax4.scatter(steps[corrective], severity[corrective], c='orange', s=60, alpha=1.0, marker='*',
                label='Self-Fixing!', zorder=5, edgecolors='darkorange', linewidths=1)
    ax4.set_xlabel('Step'); ax4.set_ylabel('Severity'); ax4.set_title('Severity Reduction (Self-Fixing)')
    ...
    for i in range(len(beta_N)-1):
        if violations[i] or violations[i+1]:
            ...
            ax5.plot([beta_N[i], beta_N[i+1]], [q95[i], q95[i+1]], color=color, alpha=alpha, linewidth=1.5, zorder=2)
        else:
            ax5.plot([beta_N[i], beta_N[i+1]], [q95[i], q95[i+1]], color='green', alpha=0.3, linewidth=1, zorder=1)
    ax5.scatter(beta_N[~violations], q95[~violations], c='green', s=20, alpha=0.7, label='Safe', zorder=3)
    ax5.scatter(beta_N[violations & ~corrective], q95[violations & ~corrective], c='red', s=40, alpha=0.8, marker='x',
                label='Violation', zorder=4)
    ax5.scatter(beta_N[corrective], q95[corrective], c='orange', s=80, alpha=1.0, marker='*',
                label='Self-Fixing!', zorder=5, edgecolors='darkorange', linewidths=1.5)
    ...
    ax5.add_patch(rect)
    ax5.scatter(beta_N[0], q95[0], c='blue', s=150, marker='o', label='Start', zorder=7, edgecolors='darkblue', linewidths=2)
    ax5.scatter(beta_N[-1], q95[-1], c='purple', s=150, marker='s', label='End', zorder=7, edgecolors='darkviolet', linewidths=2)
    ax5.set_xlabel('β_N'); ax5.set_ylabel('q95'); ax5.set_title('2D Trajectory: Toward Safety')
    ax5.legend(fontsize=6, loc='best'); ax5.grid(True, alpha=0.3)
    ...
    ax6.set_xlabel('Step'); ax6.set_ylabel('Cumulative Count')
    ...
    ax7.bar(steps[1:][severity_change < 0], severity_change[severity_change < 0], color='orange', alpha=0.7,
            label='Improving (Self-Fixing)')
    ax7.bar(steps[1:][severity_change >= 0], severity_change[severity_change >= 0], color='red', alpha=0.5,
            label='Worsening')
    ax7.axhline(0, color='black', linestyle='-', linewidth=1, zorder=0)
    ax7.set_xlabel('Step'); ax7.set_ylabel('Δ Severity'); ax7.set_title('Severity Change Per Step')
    ax7.legend(fontsize=7); ax7.grid(True, alpha=0.3, axis='y')

    # State distribution
    ax8 = plt.subplot(3, 3, 8)
    state_counts = {'Safe': np.sum(~violations), 'Violating': np.sum(violations & ~corrective), 'Self-Fixing': np.sum(corrective)}
    colors = ['green', 'red', 'orange']
    bars = ax8.bar(state_counts.keys(), state_counts.values(), color=colors, alpha=0.7, edgecolor='black', linewidth=1.5)
    ax8.set_ylabel('Count'); ax8.set_title('State Distribution'); ax8.grid(True, alpha=0.3, axis='y')
    for bar in bars:
        h = bar.get_height()
        ax8.text(bar.get_x() + bar.get_width()/2., h, f'{int(h)}', ha='center', va='bottom', fontweight='bold')
    ...
    ax9.scatter(steps[corrective], distance_to_safe[corrective], c='orange', s=60, alpha=1.0, marker='*',
                label='Self-Fixing!', zorder=5)
    ax9.fill_between(steps, 0, distance_to_safe, alpha=0.2, color='purple')
    ax9.set_xlabel('Step'); ax9.set_ylabel('Distance'); ax9.set_title('Convergence to Safety')
...
# ----------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Random vs SAC agents in gymtorax with shape guard diagnostics.")
    p.add_argument("--episodes", type=int, default=10, help="Episodes for random baseline")
    p.add_argument("--episodes_sac", type=int, default=5, help="Episodes for SAC evaluation")
    p.add_argument("--model", type=str, default=None, help="Path to SAC model .zip (SB3). If omitted, SAC eval is skipped.")
    p.add_argument("--shape_penalty", type=float, default=0.1, help="Penalty coefficient when using guard")
    p.add_argument("--damp_on_violation", action="store_true", help="Enable action damping after violations")
    p.add_argument("--damp_factor", type=float, default=0.5, help="Action damping factor in (0,1)")
    p.add_argument("--deterministic", action="store_true", help="Use deterministic actions from SAC")
    p.add_argument("--interactive", action="store_true", help="Interactive per-step shape printouts (guarded runs)")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    if shape_history_random:
        ...
        print("=" * 60)
        visualize_shape_self_fixing(shape_history_random, agent_name="Random With Shape Guard")
    ...
    if args.model:
        ...
        if shape_history_sac:
            ...
            print("=" * 60)
            visualize_shape_self_fixing(shape_history_sac, agent_name="SAC With Shape Guard")
